class SeriesExplorer {
    ...
    renderLibrary() {
        ...
        listsContainer.innerHTML = lists.map((list) => {
            ...
            return `<article class="custom-list-card"><header class="custom-list-header"><div><span>${items.length} dizi</span><h3>${this.escape(list.name)}</h3>${list.description ? `<p>${this.escape(list.description)}</p>` : ""}</div><div class="custom-list-actions"><button type="button" data-share-series-list="custom:${this.escape(list.id)}" disabled title="Gizlilik nedeniyle liste paylaşımı pasiftir." style="opacity:.5;cursor:not-allowed;">Paylaş (Pasif)</button><button type="button" data-delete-series-list="${this.escape(list.id)}">Sil</button></div></header><div class="movies-grid library-grid">${items.length ? items.map((item) => this.libraryCard(item, "", list.id)).join("") : `<p class="library-empty">Bu liste henüz boş. Dizi detayından bu listeye içerik ekleyebilirsin.</p>`}</div></article>`;
        }).join("");
    }

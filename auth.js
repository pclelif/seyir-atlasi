...
function brandedEmail({ baseUrl, preview, eyebrow, title, greeting, message, buttonLabel, link, expiry, securityNote }) {
    ...
    return `<!doctype html>
<html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><title>${escapeEmailHtml(title)}</title><style>:root{color-scheme:light only;supported-color-schemes:light only}body,.email-canvas{background-color:#0b1026!important}.email-button{color:#fff!important;-webkit-text-fill-color:#fff!important}</style></head>
<body bgcolor="#0b1026" style="margin:0;padding:0;background-color:#0b1026!important;color:#cbd5e1;font-family:Arial,'Helvetica Neue',sans-serif;">
...
<table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center"><tr><td width="34" valign="middle"><a href="${safeBaseUrl}" style="display:block;text-decoration:none;"><img src="${safeBaseUrl}/images/logo-email.svg" width="34" height="34" alt="" style="display:block;width:34px;height:34px;border:0;"></a></td><td valign="middle" style="padding-left:3px;color:#cbd5e1;font-size:21px;font-weight:800;letter-spacing:-.4px;text-shadow:0 0 8px rgba(199,210,254,.4);"><a href="${safeBaseUrl}" style="color:#cbd5e1;text-decoration:none;">SeyirAtlası</a></td></tr></table>
</td></tr>
...
<div style="margin-bottom:20px;color:#94a3b8;font-size:12px;font-weight:800;letter-spacing:2px;text-transform:uppercase;">${escapeEmailHtml(eyebrow)}</div>
<h1 style="margin:0 0 18px;color:#e5e7eb;font-size:34px;line-height:1.15;letter-spacing:-1px;">${escapeEmailHtml(title)}</h1>
<p style="margin:0 0 12px;color:#cbd5e1;font-size:17px;line-height:1.65;">${escapeEmailHtml(greeting)}</p>
<p style="margin:0;color:#94a3b8;font-size:15px;line-height:1.75;">${escapeEmailHtml(message)}</p>
</td></tr>
<tr><td style="padding:12px 46px 34px;">
<table role="presentation" cellspacing="0" cellpadding="0" border="0"><tr><td bgcolor="#607ddd" style="border-radius:12px;background-color:#607ddd!important;background-image:linear-gradient(135deg,#7896ee,#526fcf);"><a class="email-button" href="${safeLink}" style="display:inline-block;padding:15px 24px;color:#fff!important;-webkit-text-fill-color:#fff!important;font-size:15px;font-weight:800;text-decoration:none;">${escapeEmailHtml(buttonLabel)} &nbsp;→</a></td></tr></table>
<p style="margin:20px 0 0;color:#94a3b8;font-size:13px;line-height:1.6;">${escapeEmailHtml(expiry)}</p>
</td></tr>
...
export async function handleAuth(request, response, url) {
    ...
    try {
        ...
        if (request.method === "PATCH" && url.pathname === "/api/auth/profile-visibility") {
            const user = await currentUser(request); if (!user) return reply(response, 401, { error: "Oturum süresi dolmuş." }), true;
            const data = await body(request); const isPublic = data.isPublic === true;
            let slug = user.profile_slug;
            if (isPublic && !slug) {
                const base = user.name.toLocaleLowerCase("tr-TR").normalize("NFKD").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 42) || "seyir-profili";
                slug = `${base}-${randomBytes(3).toString("hex")}`;
            }
            const result = await pool.query("UPDATE users SET profile_public=$1,profile_slug=$2,updated_at=NOW() WHERE id=$3 RETURNING *",[isPublic,slug,user.id]);
            return reply(response,200,{user:publicUser(result.rows[0]),url:isPublic?`${appUrl(request)}/public-profile.html?u=${slug}`:null}),true;
        }
